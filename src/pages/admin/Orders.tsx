...
const StatSkeleton = () => (
  <div className="bg-white rounded-xl border border-neutral-200 p-5 animate-pulse">
    ...
    <div className="mt-4 pt-3 border-t border-neutral-100"><div className="w-28 h-2.5 bg-neutral-100 rounded-full" /></div>
  </div>
);
const OrderSkeleton = () => (
  <div className="bg-white rounded-xl border border-neutral-200 p-5 animate-pulse">
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-8 pr-8 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm
            text-neutral-800 placeholder:text-neutral-400 focus:outline-none focus:ring-2
          ...
          <div className="space-y-2 pt-1">
            <div>
              ...
              <input
                ...
                className="w-full text-sm text-neutral-700 bg-neutral-50 border border-neutral-200 rounded-xl
                  px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-neutral-900/[0.06]
                  focus:border-neutral-300 transition-all"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-neutral-500 uppercase tracking-wider mb-1.5">
    ...
    {hasActiveFilters && (
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl
          text-sm font-medium text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 transition-all"
      >
        <X className="w-3.5 h-3.5" />
        Clear all filters
      </button>
...
const Orders = () => {
      ...
      const lines = [
        ...
        ...periodOrders.map(o => {
          ...
          return [
            ...
            (o.payments?.[0]?.payment_method??"N/A").toUpperCase(),
            o.status.toUpperCase(),
          ].join(',');
        }),
        '','=== SUMMARY ===',
        `Total Orders,${totals.orders}`,
        `Gross Revenue,${fmtExact(totals.gross)}`,
        `Total Discounts,${fmtExact(totals.discount)}`,
        `Net Revenue,${fmtExact(totals.net)}`,
            ...
            {Array.from({length:3}).map((_,i)=>(
              <div key={i} className="bg-white rounded-xl border border-neutral-200 p-4 animate-pulse space-y-3">
                ...
                {Array.from({length:4}).map((_,j)=>(
                  <div key={j} className="w-full h-9 bg-neutral-100 rounded-xl" />
                ))}
              </div>
            ))}
          </div>
  ...
  const sidebarProps: SidebarProps = {
    ...
    filter, setFilter,
    searchTerm, setSearchTerm,
    statusCounts, totalOrders: periodOrders.length,
    onClear: handleClearFilters, hasActiveFilters,
  };
    ...
    <AdminLayout title="Orders" subtitle="Manage and track all orders">
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 left-0 h-full w-72 bg-white border-r border-neutral-200
                shadow-2xl z-50 lg:hidden flex flex-col overflow-hidden"
            >
      ...
      <div className="flex gap-6">
        ...
        <aside className="hidden lg:block w-60 flex-shrink-0">
          <div className="sticky top-4 bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-4 py-3.5 border-b border-neutral-100">
              ...
              <h3 className="text-sm font-semibold text-neutral-900">Filters</h3>
              {hasActiveFilters && (
                <span className="w-2 h-2 rounded-full bg-amber-400 ml-auto flex-shrink-0" />
              )}
            </div>
            <div className="p-4">
              <SidebarFilters {...sidebarProps} />
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          {/* Top bar: mobile filter toggle + export */}
          <div className="flex items-center justify-between gap-3">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden flex items-center gap-2 px-3 py-2 bg-white border border-neutral-200
                rounded-xl text-sm font-medium text-neutral-700 shadow-sm hover:bg-neutral-50 transition-colors"
            >
            ...
            {/* Period label (desktop only — sidebar has the selector) */}
            <p className="hidden lg:block text-sm font-semibold text-neutral-700">
              {periodLabel}
            </p>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-neutral-900 hover:bg-neutral-800
                disabled:opacity-50 text-white text-sm font-medium rounded-xl transition-colors shadow-sm ml-auto"
            >
              <Download className={`w-4 h-4 ${isExporting ? 'animate-bounce' : ''}`} />
              {isExporting ? 'Exporting…' : `Export (${periodOrders.length})`}
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-xl border border-neutral-200 p-5 shadow-sm hover:shadow-md hover:border-neutral-300 transition-all">
              ...
              <div className="mt-4 pt-3 border-t border-neutral-100">
                <p className="text-[11px] text-neutral-400">
                  ...
                  {' '}· <span className="text-emerald-600 font-semibold">−KES {fmt(currStats.totalDiscount)}</span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-neutral-200 p-5 shadow-sm hover:shadow-md hover:border-neutral-300 transition-all">
              ...
              <p className="text-sm text-neutral-400 mt-1">Avg. order (net)</p>
              <div className="mt-4 pt-3 border-t border-neutral-100">
                <p className="text-[11px] text-neutral-400">
                ...
                >
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-4">
                    ...
                    <div className="flex items-center gap-2.5 flex-shrink-0 w-full sm:w-auto">
                      <p className="text-sm font-bold text-neutral-900 tabular-nums">KES {fmt(total)}</p>
                      <select
                        value={order.status}
                        onChange={e => handleStatusChange(order.id, e.target.value)}
                        disabled={updatingId === order.id}
                        className={[
                          'text-xs font-semibold border rounded-lg px-2 py-1.5 outline-none transition-all cursor-pointer disabled:opacity-50',
                        ...
                        <div className="border-t border-neutral-100 px-5 pt-3 pb-4 space-y-2">
                            ...
                            return (
                              <div key={item.id ?? i} className="flex items-center justify-between gap-4 py-1.5">
                                <div className="flex items-center gap-3 min-w-0">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-[11px] text-neutral-400">
                                      ...
                                      {discount > 0 && <span className="text-emerald-600 ml-1">(−KES {fmt(discount)} ea)</span>}
                                    </p>
                                  </div>
                                </div>
                                <div className="text-right flex-shrink-0">
                                  ...
                                  <p className="text-sm font-semibold text-neutral-900 tabular-nums">KES {fmt(net)}</p>
                                </div>
                              </div>
                            );
                          })}
                          <div className="pt-3 mt-1 border-t border-neutral-100 space-y-1">
                            ...
                            <div className="flex items-center justify-between pt-1">
                              ...
                              <p className="text-sm font-bold text-neutral-900 tabular-nums">Total: KES {fmt(total)}</p>
                            </div>
                          </div>
                        </div>
                      </motion.div>
