...
const RealTimeNotifications: React.FC = () => {
  ...
  // ← new: tracks whether we're in mobile viewport (< 768px)
  ...
  // A plain render function (not a component) so it shares the parent's state
  // without extra prop-passing or component extraction.
  const renderPanelContent = () => (
    <>
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="px-4 sm:px-5 pt-4 sm:pt-5 pb-3.5 sm:pb-4 border-b border-neutral-100 flex-shrink-0">

        {/* Title + connection badge + close */}
  ...
  return (
    <>
      ...
      {/* Mobile backdrop — separate AnimatePresence so each can exit independently */}
      <AnimatePresence>
        {isOpen && isMobile && (
          <motion.div
            key="mobile-backdrop"
